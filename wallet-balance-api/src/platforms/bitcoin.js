...
exports.sendTransaction = async (data) => {
  ...
  let signatures = '';
  let totalLength = 4;
  let length = data.tx.signatures[0].r.length / 2;
  if (length < 32) errors.push('"r" value can not be less than 32 bytes');
  totalLength += length;
  length = length.toString(16);
  signatures += `02${length}`;
  signatures += data.tx.signatures[0].r;
  length = data.tx.signatures[0].s.length / 2;
  if (length < 32) errors.push('"s" value can not be less than 32 bytes');
  totalLength += length;
  length = length.toString(16);
  signatures += `02${length}`;
  signatures += data.tx.signatures[0].s;
  if (errors.length) {
    return { success: false, errors: errors };
  }

  signatures = `30${totalLength.toString(16)}${signatures}`;

  const signedTx = {
    tx: data.tx.tx,
    tosign: data.tx.tosign,
    signatures: [ signatures ],
    pubkeys: data.tx.pubkeys,
  }
